...
pub fn start_daemon(global_state: GlobalState) -> Result<()> {
    ...
    // Add providers from new configuration
    if !global_state
        .config
        .general_options
        .providers_config
        ...
        .is_empty()
    {
        for provider_config in &global_state
            .config
            .general_options
            .providers_config
            .providers
    ...
    // Add providers from legacy configuration (can coexist with new config)
        ...
        .clone()
    {
        if added_provider_types.contains(&ProviderType::DigitalOcean) {
            warn!("DigitalOcean provider already configured via [[providers]] - skipping legacy digital_ocean_token configuration to prevent duplicate provider instances");
        } else {
    ...
    if dns_providers.is_empty() {
        bail!("At least one DNS provider must be configured. Use either the new [[providers]] configuration or legacy token fields.");
    }
