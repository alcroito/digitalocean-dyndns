...
impl Updater {
    ...
    fn attempt_update_for_record(
        ...
        // Validate that we have providers configured
        ...
        let all_provider_types: Vec<ProviderType> = self
            .dns_providers
            .iter()
            .map(|p| p.provider_type())
            .collect();

        // Filter providers based on record configuration
        for provider in &self.dns_providers {
            // Check if this record should be updated on this provider
            if !record_to_update.should_update_on(provider.provider_type(), &all_provider_types) {
                warn!(
...
pub fn get_record_to_update<'a>(
    ...
    trace!(
        ...
        record_to_update.hostname_part,
        record_to_update.record_type
    );
    ...
    records
        ...
        .iter()
        .find(|&record| {
            // Match by hostname and record type
            // The name field is normalized to hostname-only format by all providers
