...
use crate::{config::app_config::UpdateInterval, digital_ocean_token::SecretDigitalOceanToken};
...
pub struct CommonArgs {
    ...
    #[arg(short = 't', long, value_parser = crate::digital_ocean_token::parse_secret_token)]
...
fn get_cli_command_definition_base() -> Command {
    Command::new("DigitalOcean/Hetzner dynamic dns updater")
        .version(crate_version!())
        .about("Updates DigitalOcean/Hetzner domain records to point to the current machine's public IP")
        .next_line_help(true)
        .override_usage(
            "\
    Simple config mode:
